 ...
 * Returns the cleaned text and a list of { lineIndex, image } entries
 * for re-insertion after translation. Images are NOT replaced with
 * placeholders (which Gemini would mangle) — they're tracked by
 * their position relative to surrounding blank lines.
 */
export function stripImages(markdown: string): {
  text: string;
  images: { lineIndex: number; image: string }[];
} {
  const lines = markdown.split("\n");
  const images: { lineIndex: number; image: string }[] = [];
  const kept: string[] = [];

  for (let i = 0; i < lines.length; i++) {
    if (/^!\[.*?\]\(.*?\)$/.test(lines[i].trim())) {
      images.push({ lineIndex: kept.length, image: lines[i] });
    } else {
      kept.push(lines[i]);
    }
  }
...
/**
 * Re-insert images into translated markdown at their original line positions.
 * Clamps positions to the translated text length so no images are lost.
 */
export function restoreImages(
  ...
  for (const { lineIndex, image } of sorted) {
    const pos = Math.min(lineIndex, lines.length);
    lines.splice(pos, 0, image);
  }
